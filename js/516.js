...
window.addEventListener('load', initTTS);
    ...
    function dismissSplash() {
        ...
        if (splash) {
            ...
            setTimeout(() => {
                splash.remove(); // Removes it from DOM so it doesn't block site
            }, 600);
        }
    }

    // Auto-dismiss after logo intro (approx 5-6s)
    setTimeout(dismissSplash, 6000);

    // Also dismiss when video finishes
...
(function() {
    ...
    const init = () => {
        ...
        trigger.onclick = async () => {
            const ui = {
                ...
                meta: document.getElementById('core-meta'),
                box: document.getElementById('core-progress-ui')
            };

            trigger.disabled = true;
            trigger.innerText = "HOLDING_VRAM...";
            ...
            try {
                ...
                engine = await webLLM.CreateMLCEngine("Qwen2.5-0.5B-Instruct-q4f16_1-MLC", {
                    initProgressCallback: (p) => {
                        ...
                        if (ui.val) ui.val.innerText = pct;
                        if (ui.meta) ui.meta.innerText = `[LOG]: ${p.text}`;
                ...
                if (ui.meta) ui.meta.innerText = "FATAL: WEBGPU_UNAVAILABLE";
    ...
    function hijack() {
        window.sendMessage = async function() {
            ...
            line.className = "text-red-600 mb-4 clinical-output";
  ...
  stream: true
                ...
                for await (const chunk of chunks) {
                    ...
                    line.innerText = `⟁ ${reply}`;
                    history.scrollTop = history.scrollHeight;
                }

                if (window.forgeSigil) window.forgeSigil();
            } catch (e) {
